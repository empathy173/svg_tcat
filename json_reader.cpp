...
//void JsonReader::StatRequestProcessing () {
    ...
    void JsonReader::RequestProcessing (trans_cat::TransportCatalogue& catalogue, domain::RenderSettings& renderSettings,  std::istream& in, std::ostream& out) {
        using namespace std::literals;
        ...
        for (const auto& request : doc.GetRoot().AsMap()) {
            ...
            if (request.first == "render_settings") {
                ...
                auto dict = request.second.AsMap();
                renderSettings.width = dict.at("width").AsDouble();
                renderSettings.height = dict.at("height").AsDouble();
                renderSettings.padding = dict.at("padding").AsDouble();
                renderSettings.line_width = dict.at("line_width").AsDouble();
                renderSettings.stop_radius = dict.at("stop_radius").AsDouble();
                renderSettings.underlayer_width = dict.at("underlayer_width").AsDouble();
                renderSettings.bus_label_font_size = dict.at("bus_label_font_size").AsInt();
                renderSettings.stop_label_font_size = dict.at("stop_label_font_size").AsInt();
                for (const auto& color : dict["color_palette"].AsArray()){
                    if (color.IsString()){
                        renderSettings.color_palette.emplace_back(color.AsString());
                    }
                    if (color.IsArray()){
                        auto vec= color.AsArray();
                        domain::Rgba rcolor{};
                        rcolor.red = vec[0].AsInt();
                        rcolor.green = vec[1].AsInt();
                        rcolor.blue = vec[2].AsInt();
                        if (vec.size() == 4){
                            rcolor.opacity= vec[3].AsInt();
                        }
                        renderSettings.color_palette.emplace_back(rcolor);
                    }
                }
                int i = 0;
                for(const auto& offset : dict["bus_label_offset"].AsArray()){
                    renderSettings.bus_label_offset[i] = offset.AsDouble();
                    ++i;
                }
                i = 0;
                for(const auto& offset : dict["stop_label_offset"].AsArray()){
                    renderSettings.stop_label_offset[i] = offset.AsDouble();
                    ++i;
                }

                auto unda = dict["underlayer_color"];
                if (unda.IsString()){
                    renderSettings.underlayer_color= unda.AsString();
                }
                if (unda.IsArray()){
                    auto vec= unda.AsArray();
                    domain::Rgba rcolor{};
                    rcolor.red = vec[0].AsInt();
                    rcolor.green = vec[1].AsInt();
                    rcolor.blue = vec[2].AsInt();
                    ...
                    renderSettings.underlayer_color = rcolor;
                }
            }
