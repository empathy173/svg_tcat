...
public:
	...
	void AddBus (const std::string& bus_name, std::vector<domain::Stop*>& stops_for_bus, bool is_roundtrip);
...
private:
	...
	// double 	GetBusRouteLength 	 (const domain::Bus& bus) const;
	std::tuple<double, double> 	GetBusRouteLength 	 (const domain::Bus& bus) const;
